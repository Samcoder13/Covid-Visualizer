...
og_data.forEach( function(d){ move_dict.set( d.fips, d.cpi) });
        ...
        selection.each(function(data) {
            ...
            function toolTipHTML(dict, mode) 
            {
                ...
                for (var key in dict) {
                    ...
                    var value;
                 //change percentage format and apply commas in figures
                    if (!isNaN(dict[key])) {
                        if (dict[key] % 1 == 0)
                            value = numberWithCommas(dict[key]);
                        else if (key.indexOf("cpi") !== -1)
            ...
            var state_map = {"State": state, "Cases": cases_total, "Population": pop_total, "Cases Per Capita": cases_total / pop_total};
            ...
            //data in circle -->donut  inside circle
            function toolTip(selection) {
                svg.append('text')
                    .attr('class', 'toolCircle')
                    .attr('dy', -25) //can adjust this to adjust text vertical alignment in tooltip
